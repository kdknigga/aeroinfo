...
"""
SQLAlchemy models for NASR APT records.

This module contains the Airport, Runway and related ORM models used by
the application.
"""
...
class Airport(Base):
    """Airport model representing NASR APT records."""

    __tablename__ = "airports"

    # L A N D I N G   F A C I L I T Y   D A T A
    # L AN 0011 00004  DLID    LANDING FACILITY SITE NUMBER
    facility_site_number: Mapped[str] = mapped_column(String(11), primary_key=True)
    # L AN 0013 00015  DLID    LANDING FACILITY TYPE
    facility_type: Mapped[str | None] = mapped_column(String(13))
    # L AN 0004 00028  E7      LOCATION IDENTIFIER
    faa_id: Mapped[str | None] = mapped_column(String(4), index=True)
    ...
    field_office: Mapped[str | None] = mapped_column(String(4))
    # L AN 0002 00049  A4      ASSOCIATED STATE POST OFFICE CODE
    state_code: Mapped[str | None] = mapped_column(String(2))
    # L AN 0020 00051  A4      ASSOCIATED STATE NAME
    state_name: Mapped[str | None] = mapped_column(String(20))
    # L AN 0021 00071  A5      ASSOCIATED COUNTY (OR PARISH) NAME
    county: Mapped[str | None] = mapped_column(String(21))
    county_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0002 00092  A5      ASSOCIATED COUNTY'S STATE (POST OFFICE CODE)
    countys_state: Mapped[str | None] = mapped_column(String(2))
    # L AN 0040 00094  A1      ASSOCIATED CITY NAME
    city: Mapped[str | None] = mapped_column(String(40))
    city_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0050 00134  A2      OFFICIAL FACILITY NAME
    name: Mapped[str | None] = mapped_column(String(50))
    name_remark: Mapped[str | None] = mapped_column(String(1500))

    # OWNERSHIP DATA
    # L AN 0002 00184  A10     AIRPORT OWNERSHIP TYPE
    ownership_type: Mapped[enums.OwnershipTypeEnum | None] = mapped_column(
        Enum(enums.OwnershipTypeEnum)
    )
    ownership_type_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0002 00186  A18     FACILITY USE
    facility_use: Mapped[enums.FacilityUseEnum | None] = mapped_column(
        Enum(enums.FacilityUseEnum)
    )
    facility_use_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0035 00188  A11     FACILITY OWNER'S NAME
    owners_name: Mapped[str | None] = mapped_column(String(35))
    owners_name_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0072 00223  A12     OWNER'S ADDRESS
    owners_address: Mapped[str | None] = mapped_column(String(72))
    owners_address_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0045 00295  A12A    OWNER'S CITY, STATE AND ZIP CODE
    owners_city_state_zip: Mapped[str | None] = mapped_column(String(45))
    owners_city_state_zip_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0016 00340  A13     OWNER'S PHONE NUMBER
    owners_phone: Mapped[str | None] = mapped_column(String(16))
    owners_phone_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0035 00356  A14     FACILITY MANAGER'S NAME
    managers_name: Mapped[str | None] = mapped_column(String(35))
    managers_name_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0072 00391  A15     MANAGER'S ADDRESS
    managers_address: Mapped[str | None] = mapped_column(String(72))
    managers_address_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0045 00463  A15A    MANAGER'S CITY, STATE AND ZIP CODE
    managers_city_state_zip: Mapped[str | None] = mapped_column(String(45))
    managers_city_state_zip_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0016 00508  A16     MANAGER'S PHONE NUMBER
    managers_phone: Mapped[str | None] = mapped_column(String(16))
    managers_phone_remark: Mapped[str | None] = mapped_column(String(1500))

    # GEOGRAPHIC DATA
    # L AN 0015 00524  A19     AIRPORT REFERENCE POINT LATITUDE (FORMATTED)
    latitude_dms: Mapped[str | None] = mapped_column(String(15))
    latitude_dms_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0012 00539  A19S    AIRPORT REFERENCE POINT LATITUDE (SECONDS)
    latitude_secs: Mapped[str | None] = mapped_column(String(12))
    # L AN 0015 00551  A20     AIRPORT REFERENCE POINT LONGITUDE (FORMATTED)
    longitude_dms: Mapped[str | None] = mapped_column(String(15))
    longitude_dms_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0012 00566  A20S    AIRPORT REFERENCE POINT LONGITUDE (SECONDS)
    longitude_secs: Mapped[str | None] = mapped_column(String(12))
    # L AN 0001 00578  A19A    AIRPORT REFERENCE POINT DETERMINATION METHOD
    coords_method: Mapped[enums.DeterminationMethodEnum | None] = mapped_column(
        Enum(enums.DeterminationMethodEnum)
    )
    coords_method_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0007 00579  A21     AIRPORT ELEVATION  (NEAREST TENTH OF A FOOT MSL)
    elevation: Mapped[float | None] = mapped_column(Float(1))
    elevation_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00586  A21     AIRPORT ELEVATION DETERMINATION METHOD
    elevation_method: Mapped[enums.DeterminationMethodEnum | None] = mapped_column(
        Enum(enums.DeterminationMethodEnum)
    )
    # L AN 0003 00587  E28     MAGNETIC VARIATION AND DIRECTION
    mag_variation: Mapped[str | None] = mapped_column(String(3))
    ...
    pattern_alt: Mapped[int | None] = mapped_column(Integer)
    pattern_alt_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0030 00598  A7      AERONAUTICAL SECTIONAL CHART ON WHICH FACILITY APPEARS
    sectional: Mapped[str | None] = mapped_column(String(30))
    sectional_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0002 00628  A3      DISTANCE FROM CENTRAL BUSINESS DISTRICT OF THE ASSOCIATED CITY TO THE AIRPORT
    distance_from_city: Mapped[int | None] = mapped_column(Integer)
    distance_from_city_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0003 00630  A3      DIRECTION OF AIRPORT FROM CENTRAL BUSINESS
    direction_from_city: Mapped[str | None] = mapped_column(String(3))
    # R AN 0005 00633  A22     LAND AREA COVERED BY AIRPORT (ACRES)
    land_area: Mapped[int | None] = mapped_column(Integer)
    land_area_remark: Mapped[str | None] = mapped_column(String(1500))

    # FAA SERVICES
    # L AN 0004 00638  E146A   BOUNDARY ARTCC IDENTIFIER
    boundary_artcc_id: Mapped[str | None] = mapped_column(String(4))
    # L AN 0003 00642  E146B   BOUNDARY ARTCC (FAA) COMPUTER IDENTIFIER
    boundary_artcc_computer_id: Mapped[str | None] = mapped_column(String(3))
    # L AN 0030 00645  E146C   BOUNDARY ARTCC NAME
    boundary_artcc_name: Mapped[str | None] = mapped_column(String(30))
    # L AN 0004 00675  E156A   RESPONSIBLE ARTCC IDENTIFIER
    responsible_artcc_id: Mapped[str | None] = mapped_column(String(4))
    responsible_artcc_id_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0003 00679  E156B   RESPONSIBLE ARTCC (FAA) COMPUTER IDENTIFIER
    responsible_artcc_computer_id: Mapped[str | None] = mapped_column(String(3))
    # L AN 0030 00682  E156C   RESPONSIBLE ARTCC NAME
    responsible_artcc_name: Mapped[str | None] = mapped_column(String(30))
    # L AN 0001 00712  A87     TIE-IN FSS PHYSICALLY LOCATED ON FACILITY
    tie_in_fss_local: Mapped[bool | None] = mapped_column(Boolean)
    # L AN 0004 00713  A86     TIE-IN FLIGHT SERVICE STATION (FSS) IDENTIFIER
    tie_in_fss_id: Mapped[str | None] = mapped_column(String(4))
    tie_in_fss_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0030 00717  A86     TIE-IN FSS NAME
    tie_in_fss_name: Mapped[str | None] = mapped_column(String(30))
    # L AN 0016 00747  A88     LOCAL PHONE NUMBER FROM AIRPORT TO FSS
    fss_local_phone: Mapped[str | None] = mapped_column(String(16))
    # L AN 0016 00763  A89     TOLL FREE PHONE NUMBER FROM AIRPORT TO FSS
    fss_toll_free_phone: Mapped[str | None] = mapped_column(String(16))
    # L AN 0004 00779  A86A    ALTERNATE FSS IDENTIFIER
    alternate_fss_id: Mapped[str | None] = mapped_column(String(4))
    # L AN 0030 00783  A86A    ALTERNATE FSS NAME
    alternate_fss_name: Mapped[str | None] = mapped_column(String(30))
    # L AN 0016 00813  E3A     TOLL FREE PHONE NUMBER FROM AIRPORT TO ALTERNATE FSS
    alternate_fss_toll_free_phone: Mapped[str | None] = mapped_column(String(16))
    # L AN 0004 00829  E2B     IDENTIFIER OF THE FACILITY RESPONSIBLE FOR ISSUING NOTICES TO AIRMEN
    notam_facility: Mapped[str | None] = mapped_column(String(4))
    # L AN 0001 00833  E139    AVAILABILITY OF NOTAM 'D' SERVICE AT AIRPORT
    notam_d_available: Mapped[bool | None] = mapped_column(Boolean)
    ...
    status: Mapped[enums.AirportStatusEnum | None] = mapped_column(
        Enum(enums.AirportStatusEnum)
    )
    # L AN 0015 00843  A26     AIRPORT ARFF CERTIFICATION TYPE AND DATE
    arff_certification: Mapped[str | None] = mapped_column(
        String(15)
    )  # Candidate for parsing and breaking into multiple columns
    arff_certification_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )  # Candidate for parsing and breaking into multiple columns
    # L AN 0007 00858  A25     NPIAS/FEDERAL AGREEMENTS CODE
    npias_federal_agreements: Mapped[str | None] = mapped_column(
        String(7)
    )  # Candidate for parsing and breaking into multiple columns
    npias_federal_agreements_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0013 00865  E111    AIRPORT AIRSPACE ANALYSIS DETERMINATION
    airspace_analysis: Mapped[str | None] = mapped_column(String(13))
    airspace_analysis_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00878  E79     FACILITY HAS BEEN DESIGNATED BY THE U.S. TREASURY AS AN INTERNATIONAL AIRPORT OF ENTRY FOR CUSTOMS
    airport_of_entry: Mapped[bool | None] = mapped_column(Boolean)
    airport_of_entry_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00879  E80     FACILITY HAS BEEN DESIGNATED BY THE U.S. TREASURY AS A CUSTOMS LANDING RIGHTS AIRPORT
    customs_landing_rights: Mapped[bool | None] = mapped_column(Boolean)
    customs_landing_rights_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00880  E115    FACILITY HAS MILITARY/CIVIL JOINT USE AGREEMENT THAT ALLOWS CIVIL OPERATIONS AT A MILITARY AIRPORT
    military_civil_join_use: Mapped[bool | None] = mapped_column(Boolean)
    military_civil_join_use_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00881  E116    AIRPORT HAS ENTERED INTO AN AGREEMENT THAT GRANTS LANDING RIGHTS TO THE MILITARY
    military_landing_rights: Mapped[bool | None] = mapped_column(Boolean)
    military_landing_rights_remark: Mapped[str | None] = mapped_column(String(1500))

    # AIRPORT INSPECTION DATA
    # L AN 0002 00882  E155    AIRPORT INSPECTION METHOD
    inspection_method: Mapped[enums.AirportInspectionMethodEnum | None] = mapped_column(
        Enum(enums.AirportInspectionMethodEnum)
    )
    # L AN 0001 00884  A111    AGENCY/GROUP PERFORMING PHYSICAL INSPECTION
    agency_performing_inspection: Mapped[
        enums.AgencyPerformingInspectionEnum | None
    ] = mapped_column(Enum(enums.AgencyPerformingInspectionEnum))
    agency_performing_inspection_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # L AN 0008 00885  A112    LAST PHYSICAL INSPECTION DATE (MMDDYYYY)
    last_inspection_date: Mapped[datetime.date | None] = mapped_column(Date)
    last_inspection_date_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0008 00893  A113    LAST DATE INFORMATION REQUEST WAS COMPLETED
    last_information_request_complete_date: Mapped[datetime.date | None] = (
        mapped_column(Date)
    )

    # AIRPORT SERVICES
    # L AN 0040 00901  A70     FUEL TYPES AVAILABLE FOR PUBLIC USE
    fuel_available: Mapped[str | None] = mapped_column(
        String(40)
    )  # Candidate for parsing and breaking into multiple columns
    fuel_available_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0005 00941  A71     AIRFRAME REPAIR SERVICE AVAILABILITY/TYPE
    airframe_repair_service: Mapped[str | None] = mapped_column(String(5))
    airframe_repair_service_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0005 00946  A72     POWER PLANT (ENGINE) REPAIR AVAILABILITY/TYPE
    power_plant_repair_service: Mapped[str | None] = mapped_column(String(5))
    power_plant_repair_service_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0008 00951  A73     TYPE OF BOTTLED OXYGEN AVAILABLE
    bottled_oxygen: Mapped[str | None] = mapped_column(String(8))
    bottled_oxygen_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0008 00959  A74     TYPE OF BULK OXYGEN AVAILABLE
    bulk_oxygen: Mapped[str | None] = mapped_column(String(8))
    bulk_oxygen_remark: Mapped[str | None] = mapped_column(String(1500))

    # AIRPORT FACILITIES
    # L AN 0007 00967  A81     AIRPORT LIGHTING SCHEDULE
    lighting_schedule: Mapped[str | None] = mapped_column(String(7))
    lighting_schedule_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0007 00974  A81     BEACON LIGHTING SCHEDULE
    beacon_schedule: Mapped[str | None] = mapped_column(String(7))
    beacon_schedule_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00981  A85     AIR TRAFFIC CONTROL TOWER LOCATED ON AIRPORT
    towered_airport: Mapped[bool | None] = mapped_column(Boolean)
    # L AN 0007 00982  A82     UNICOM FREQUENCY AVAILABLE AT THE AIRPORT
    unicom: Mapped[str | None] = mapped_column(String(7))
    unicom_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0007 00989  E100    COMMON TRAFFIC ADVISORY FREQUENCY (CTAF)
    ctaf: Mapped[str | None] = mapped_column(String(7))
    ctaf_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0004 00996  A84     SEGMENTED CIRCLE AIRPORT MARKER SYSTEM ON THE AIRPORT
    segmented_circle_available: Mapped[enums.SegmentedCircleEnum | None] = (
        mapped_column(Enum(enums.SegmentedCircleEnum))
    )
    segmented_circle_available_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0003 01000  A80     LENS COLOR OF OPERABLE BEACON LOCATED ON THE AIRPORT
    beacon_color: Mapped[enums.BeaconColorEnum | None] = mapped_column(
        Enum(enums.BeaconColorEnum)
    )
    beacon_color_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 01003  A24     LANDING FEE CHARGED TO NON-COMMERCIAL USERS
    noncommerical_landing_fee: Mapped[bool | None] = mapped_column(Boolean)
    noncommerical_landing_fee_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 01004  NONE    A "Y" IN THIS FIELD INDICATES THAT THE LANDING FACILITY IS USED FOR MEDICAL PURPOSES
    landing_facility_used_for_medical_purposes: Mapped[bool | None] = mapped_column(
        Boolean
    )

    # BASED AIRCRAFT
    # R  N 0003 01005  A90     SINGLE ENGINE GENERAL AVIATION AIRCRAFT
    based_general_aviation_single_engine_airplanes: Mapped[int | None] = mapped_column(
        Integer
    )
    based_general_aviation_single_engine_airplanes_remark: Mapped[str | None] = (
        mapped_column(String(1500))
    )
    # R  N 0003 01008  A91     MULTI ENGINE GENERAL AVIATION AIRCRAFT
    based_general_aviation_multi_engine_airplanes: Mapped[int | None] = mapped_column(
        Integer
    )
    based_general_aviation_multi_engine_airplanes_remark: Mapped[str | None] = (
        mapped_column(String(1500))
    )
    # R  N 0003 01011  A92     JET ENGINE GENERAL AVIATION AIRCRAFT
    based_general_aviation_jet_engine_airplanes: Mapped[int | None] = mapped_column(
        Integer
    )
    based_general_aviation_jet_engine_airplanes_remark: Mapped[str | None] = (
        mapped_column(String(1500))
    )
    # R  N 0003 01014  A93     GENERAL AVIATION HELICOPTER
    based_general_aviation_helicopters: Mapped[int | None] = mapped_column(Integer)
    based_general_aviation_helicopters_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # R  N 0003 01017  A94     OPERATIONAL GLIDERS
    based_gliders: Mapped[int | None] = mapped_column(Integer)
    based_gliders_remark: Mapped[str | None] = mapped_column(String(1500))
    # R  N 0003 01020  A95     OPERATIONAL MILITARY AIRCRAFT (INCLUDING HELICOPTERS)
    based_military_aircraft: Mapped[int | None] = mapped_column(Integer)
    based_military_aircraft_remark: Mapped[str | None] = mapped_column(String(1500))
    # R  N 0003 01023  A96     ULTRALIGHT AIRCRAFT
    based_ultralight_aircraft: Mapped[int | None] = mapped_column(Integer)
    based_ultralight_aircraft_remark: Mapped[str | None] = mapped_column(String(1500))

    # ANNUAL OPERATIONS
    # R  N 0006 01026  A100    COMMERCIAL SERVICES
    annual_ops_commercial: Mapped[int | None] = mapped_column(Integer)
    annual_ops_commercial_remark: Mapped[str | None] = mapped_column(String(1500))
    # R  N 0006 01032  A101    COMMUTER SERVICES
    annual_ops_commuter: Mapped[int | None] = mapped_column(Integer)
    annual_ops_commuter_remark: Mapped[str | None] = mapped_column(String(1500))
    # R  N 0006 01038  A102    AIR TAXI
    annual_ops_air_taxi: Mapped[int | None] = mapped_column(Integer)
    annual_ops_air_taxi_remark: Mapped[str | None] = mapped_column(String(1500))
    # R  N 0006 01044  A103    GENERAL AVIATION LOCAL OPERATIONS
    annual_ops_general_aviation_local: Mapped[int | None] = mapped_column(Integer)
    annual_ops_general_aviation_local_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # R  N 0006 01050  A104    GENERAL AVIATION ITINERANT OPERATIONS
    annual_ops_general_aviation_itinerant: Mapped[int | None] = mapped_column(Integer)
    annual_ops_general_aviation_itinerant_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # R  N 0006 01056  A105    MILITARY AIRCRAFT OPERATIONS
    annual_ops_military: Mapped[int | None] = mapped_column(Integer)
    annual_ops_military_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0010 01062  NONE    12-MONTH ENDING DATE ON WHICH ANNUAL OPERATIONS DATA IN ABOVE SIX FIELDS IS BASED (MM/DD/YYYY)
    annual_ops_end_of_measurement_period: Mapped[datetime.date | None] = mapped_column(
        Date
    )

    # ADDITIONAL AIRPORT DATA
    # L AN 0016 01072  NONE    AIRPORT POSITION SOURCE
    position_source: Mapped[str | None] = mapped_column(String(16))
    # L AN 0010 01088  NONE    AIRPORT POSITION SOURCE DATE (MM/DD/YYYY)
    position_date: Mapped[datetime.date | None] = mapped_column(Date)
    # L AN 0016 01098  NONE    AIRPORT ELEVATION SOURCE
    elevation_source: Mapped[str | None] = mapped_column(String(16))
    # L AN 0010 01114  NONE    AIRPORT ELEVATION SOURCE DATE (MM/DD/YYYY)
    elevation_date: Mapped[datetime.date | None] = mapped_column(Date)
    # L AN 0001 01124  NONE    CONTRACT FUEL AVAILABLE
    contract_fuel_available: Mapped[bool | None] = mapped_column(Boolean)
    # L AN 0012 01125  A75     TRANSIENT STORAGE FACILITIES
    transient_storage_facilities: Mapped[str | None] = mapped_column(String(12))
    transient_storage_facilities_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # L AN 0071 01137  A76     OTHER AIRPORT SERVICES AVAILABLE
    other_services_available: Mapped[str | None] = mapped_column(String(71))
    other_services_available_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0003 01208  A83     WIND INDICATOR
    wind_indicator: Mapped[enums.SegmentedCircleEnum | None] = mapped_column(
        Enum(enums.SegmentedCircleEnum)
    )
    wind_indicator_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0007 01211  NONE    ICAO IDENTIFIER
    icao_id: Mapped[str | None] = mapped_column(String(7), index=True)
    ...
    def to_dict(self, include: list[str] | None = None) -> dict[str, object]:
        ...
        The optional ``include`` list can be used to include additional groups
        of attributes such as "demographic" or "runways".
        """
        _include = include or []
        ...
        if "runways" in _include:
            ...
            result["runways"] = [runway.to_dict() for runway in runways]

        if "remarks" in _include:
            remarks = list(self.remarks)
            result["remarks"] = [remark.remark for remark in remarks]

        if "attendance" in _include:
            attendance_schedules = list(self.attendance_schedules)
            result["attendance"] = [
                attsched.attendance_schedule for attsched in attendance_schedules
            ]
...
class Runway(Base):
    """Model for a runway attached to an Airport."""

    __tablename__ = "runways"

    # F A C I L I T Y   R U N W A Y   D A T A
    # L AN 0011 00004  DLID    LANDING FACILITY SITE NUMBER
    facility_site_number: Mapped[str] = mapped_column(
        String(11), ForeignKey("airports.facility_site_number"), primary_key=True
    )
    # L AN 0007 00017  A30     RUNWAY IDENTIFICATION
    name: Mapped[str] = mapped_column(String(7), primary_key=True)
    name_remark: Mapped[str | None] = mapped_column(String(1500))

    # COMMON RUNWAY DATA
    # R AN 0005 00024  A31     PHYSICAL RUNWAY LENGTH (NEAREST FOOT)
    length: Mapped[int | None] = mapped_column(Integer)
    length_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0004 00029  A32     PHYSICAL RUNWAY WIDTH (NEAREST FOOT)
    width: Mapped[int | None] = mapped_column(Integer)
    width_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0012 00033  A33     RUNWAY SURFACE TYPE AND CONDITION
    surface_type_condition: Mapped[str | None] = mapped_column(String(12))
    surface_type_condition_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0005 00045  A34     RUNWAY SURFACE TREATMENT
    surface_treatment: Mapped[str | None] = mapped_column(String(5))
    surface_treatment_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0011 00050  A39     PAVEMENT CLASSIFICATION NUMBER (PCN)
    pavement_classification_number: Mapped[str | None] = mapped_column(String(11))
    pavement_classification_number_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # L AN 0005 00061  A40     RUNWAY LIGHTS EDGE INTENSITY
    edge_light_intensity: Mapped[str | None] = mapped_column(String(5))
    edge_light_intensity_remark: Mapped[str | None] = mapped_column(String(1500))

    # ADDITIONAL COMMON RUNWAY DATA
    # L AN 0016 00510  NONE    RUNWAY LENGTH SOURCE
    length_source: Mapped[str | None] = mapped_column(String(16))
    # L AN 0010 00526  NONE    RUNWAY LENGTH SOURCE DATE (MM/DD/YYYY)
    length_source_date: Mapped[datetime.date | None] = mapped_column(Date)
    # R AN 0006 00536  A35     RUNWAY WEIGHT-BEARING CAPACITY FOR Single wheel type landing gear
    weight_bearing_capacity_single_wheel: Mapped[str | None] = mapped_column(String(6))
    weight_bearing_capacity_single_wheel_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # R AN 0006 00542  A36     RUNWAY WEIGHT-BEARING CAPACITY FOR Dual wheel type landing gear
    weight_bearing_capacity_dual_wheels: Mapped[str | None] = mapped_column(String(6))
    weight_bearing_capacity_dual_wheels_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # R AN 0006 00548  A37     RUNWAY WEIGHT-BEARING CAPACITY FOR Two dual wheels in tandem type landing gear
    weight_bearing_capacity_two_dual_wheels_tandem: Mapped[str | None] = mapped_column(
        String(6)
    )
    weight_bearing_capacity_two_dual_wheels_tandem_remark: Mapped[str | None] = (
        mapped_column(String(1500))
    )
    ...
    weight_bearing_capacity_two_dual_wheels_double_tandem: Mapped[str | None] = (
        mapped_column(String(6))
    )
    weight_bearing_capacity_two_dual_wheels_double_tandem_remark: Mapped[str | None] = (
        mapped_column(String(1500))
    )
    ...
    def to_dict(self, include: list[str] | None = None) -> dict[str, object]:
        ...
        ``include`` may contain additional groups like "additional".
        """
        _include = include or []
        ...
        if "runway_ends" in _include:
            runway_ends = list(self.runway_ends)
            result["runway_ends"] = [end.to_dict() for end in runway_ends]

        return result


class RunwayEnd(Base):
    """Model for an end of a runway."""

    __tablename__ = "runway_ends"

    facility_site_number: Mapped[str] = mapped_column(String(11), primary_key=True)
    runway_name: Mapped[str] = mapped_column(String(7), primary_key=True)
    ...
    id_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0003 00069  E46     RUNWAY END TRUE ALIGNMENT
    # L AN 0003 00291  E46     RUNWAY END TRUE ALIGNMENT
    true_alignment: Mapped[int | None] = mapped_column(Integer)
    true_alignment_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0010 00072  I22     INSTRUMENT LANDING SYSTEM (ILS) TYPE
    # L AN 0010 00294  I22     INSTRUMENT LANDING SYSTEM (ILS) TYPE
    approach_type: Mapped[str | None] = mapped_column(String(10))
    ...
    right_traffic: Mapped[bool | None] = mapped_column(Boolean)
    right_traffic_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0005 00083  A42     RUNWAY MARKINGS  (TYPE)
    # L AN 0005 00305  A42     RUNWAY MARKINGS  (TYPE)
    markings_type: Mapped[enums.RunwayMarkingsTypeEnum | None] = mapped_column(
        Enum(enums.RunwayMarkingsTypeEnum)
    )
    markings_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00088  A42     RUNWAY MARKINGS  (CONDITION)
    # L AN 0001 00310  A42     RUNWAY MARKINGS  (CONDITION)
    markings_condition: Mapped[enums.RunwayMarkingsConditionEnum | None] = (
        mapped_column(Enum(enums.RunwayMarkingsConditionEnum))
    )
    ...
    threshold_crossing_height: Mapped[int | None] = mapped_column(Integer)
    threshold_crossing_height_remark: Mapped[str | None] = mapped_column(String(1500))
    ...
    visual_glide_path_angle: Mapped[float | None] = mapped_column(Float(2))
    visual_glide_path_angle_remark: Mapped[str | None] = mapped_column(String(1500))
    ...
    displaced_threshold_latitude_dms: Mapped[str | None] = mapped_column(String(15))
    displaced_threshold_latitude_dms_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    ...
    displaced_threshold_latitude_secs: Mapped[str | None] = mapped_column(String(12))
    ...
    displaced_threshold_longitude_dms: Mapped[str | None] = mapped_column(String(15))
    displaced_threshold_longitude_dms_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    ...
    displaced_threshold_longitude_secs: Mapped[str | None] = mapped_column(String(12))
    ...
    displaced_threshold_elevation: Mapped[float | None] = mapped_column(Float(1))
    ...
    displaced_threshold_length: Mapped[int | None] = mapped_column(Integer)
    displaced_threshold_length_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0007 00222  E163    ELEVATION AT TOUCHDOWN ZONE (FEET MSL)
    # R AN 0007 00444  E163    ELEVATION AT TOUCHDOWN ZONE (FEET MSL)
    touchdown_zone_elevation: Mapped[float | None] = mapped_column(Float(1))
    ...
    visual_glide_slope_indicators: Mapped[
        enums.VisualGlideSlopeIndicatorEnum | None
    ] = mapped_column(Enum(enums.VisualGlideSlopeIndicatorEnum))
    visual_glide_slope_indicators_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # L AN 0003 00234  A47     RUNWAY VISUAL RANGE EQUIPMENT (RVR)
    # L AN 0003 00456  A47     RUNWAY VISUAL RANGE EQUIPMENT (RVR)
    rvr_equipment: Mapped[enums.RVREquipmentEnum | None] = mapped_column(
        Enum(enums.RVREquipmentEnum)
    )
    rvr_equipment_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00237  A47A    RUNWAY VISIBILITY VALUE EQUIPMENT (RVV)
    # L AN 0001 00459  A47A    RUNWAY VISIBILITY VALUE EQUIPMENT (RVV)
    rvv_equipment: Mapped[bool | None] = mapped_column(Boolean)
    # L AN 0008 00238  A49     APPROACH LIGHT SYSTEM
    # L AN 0008 00460  A49     APPROACH LIGHT SYSTEM
    approach_light_system: Mapped[str | None] = mapped_column(String(8))
    approach_light_system_remark: Mapped[str | None] = mapped_column(String(1500))
    ...
    reil_availability: Mapped[bool | None] = mapped_column(Boolean)
    reil_availability_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0001 00247  A46     RUNWAY CENTERLINE LIGHTS AVAILABILITY
    # L AN 0001 00469  A46     RUNWAY CENTERLINE LIGHTS AVAILABILITY
    centerline_light_availability: Mapped[bool | None] = mapped_column(Boolean)
    centerline_light_availability_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    ...
    touchdown_lights_availability: Mapped[bool | None] = mapped_column(Boolean)
    touchdown_lights_availability_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    ...
    controlling_object_description: Mapped[str | None] = mapped_column(String(11))
    controlling_object_description_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # L AN 0004 00260  A53     CONTROLLING OBJECT MARKED/LIGHTED
    # L AN 0004 00482  A53     CONTROLLING OBJECT MARKED/LIGHTED
    controlling_object_marking: Mapped[enums.ControllingObjectMarkingEnum | None] = (
        mapped_column(Enum(enums.ControllingObjectMarkingEnum))
    )
    controlling_object_marking_remark: Mapped[str | None] = mapped_column(String(1500))
    ...
    part77_category: Mapped[str | None] = mapped_column(String(5))
    part77_category_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0002 00269  A57     CONTROLLING OBJECT CLEARANCE SLOPE
    # R AN 0002 00491  A57     CONTROLLING OBJECT CLEARANCE SLOPE
    controlling_object_clearance_slope: Mapped[int | None] = mapped_column(Integer)
    controlling_object_clearance_slope_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # R AN 0005 00271  A54     CONTROLLING OBJECT HEIGHT ABOVE RUNWAY
    # R AN 0005 00493  A54     CONTROLLING OBJECT HEIGHT ABOVE RUNWAY
    controlling_object_height_above_runway: Mapped[int | None] = mapped_column(Integer)
    controlling_object_height_above_runway_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    ...
    controlling_object_distance_from_runway: Mapped[int | None] = mapped_column(Integer)
    controlling_object_distance_from_runway_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    # L AN 0007 00281  A56     CONTROLLING OBJECT CENTERLINE OFFSET
    # L AN 0007 00503  A56     CONTROLLING OBJECT CENTERLINE OFFSET
    controlling_object_centerline_offset: Mapped[str | None] = mapped_column(String(7))
    controlling_object_centerline_offset_remark: Mapped[str | None] = mapped_column(
        String(1500)
    )
    ...
    gradient: Mapped[str | None] = mapped_column(String(5))
    gradient_remark: Mapped[str | None] = mapped_column(String(1500))
    # L AN 0004 00565  E40     RUNWAY END GRADIENT DIRECTION (UP OR DOWN)
    # L AN 0004 00856  E40     RUNWAY END GRADIENT DIRECTION (UP OR DOWN)
    gradient_direction: Mapped[str | None] = mapped_column(String(4))
    ...
    displaced_threshold_position_source: Mapped[str | None] = mapped_column(String(16))
    ...
    displaced_threshold_position_date: Mapped[datetime.date | None] = mapped_column(
        Date
    )
    # L AN 0016 00647  NONE    DISPLACED THESHOLD ELEVATION SOURCE
    # L AN 0016 00938  NONE    DISPLACED THESHOLD ELEVATION SOURCE
    displaced_threshold_elevation_source: Mapped[str | None] = mapped_column(String(16))
    ...
    displaced_threshold_elevation_date: Mapped[datetime.date | None] = mapped_column(
        Date
    )
    # L AN 0016 00673  NONE    TOUCHDOWN ZONE ELEVATION SOURCE
    # L AN 0016 00964  NONE    TOUCHDOWN ZONE ELEVATION SOURCE
    touchdown_zone_elevation_source: Mapped[str | None] = mapped_column(String(16))
    ...
    touchdown_zone_elevation_date: Mapped[datetime.date | None] = mapped_column(Date)
    # R AN 0005 00699  A60     TAKEOFF RUN AVAILABLE (TORA), IN FEET
    # R AN 0005 00990  A60     TAKEOFF RUN AVAILABLE (TORA), IN FEET
    takeoff_run_available: Mapped[int | None] = mapped_column(Integer)
    takeoff_run_available_remark: Mapped[str | None] = mapped_column(String(1500))
    # R AN 0005 00704  A61     TAKEOFF DISTANCE AVAILABLE (TODA), IN FEET
    # R AN 0005 00995  A61     TAKEOFF DISTANCE AVAILABLE (TODA), IN FEET
    takeoff_distance_available: Mapped[int | None] = mapped_column(Integer)
    ...
    accelerate_stop_distance_available: Mapped[int | None] = mapped_column(Integer)
    # R AN 0005 00714  A63     LANDING DISTANCE AVAILABLE (LDA), IN FEET
    # R AN 0005 01005  A63     LANDING DISTANCE AVAILABLE (LDA), IN FEET
    landing_distance_available: Mapped[int | None] = mapped_column(Integer)
    ...
    lahso_distance_available: Mapped[int | None] = mapped_column(Integer)
    ...
    id_of_lahso_intersecting_runway: Mapped[str | None] = mapped_column(String(7))
    ...
    description_of_lahso_entity: Mapped[str | None] = mapped_column(String(40))
    ...
    lahso_latitude_dms: Mapped[str | None] = mapped_column(String(15))
    ...
    lahso_latitude_secs: Mapped[str | None] = mapped_column(String(12))
    ...
    lahso_longitude_dms: Mapped[str | None] = mapped_column(String(15))
    ...
    lahso_longitude_secs: Mapped[str | None] = mapped_column(String(12))
    # L AN 0016 00825  NONE    LAHSO HOLD SHORT POINT LAT/LONG SOURCE
    # L AN 0016 01116  NONE    LAHSO HOLD SHORT POINT LAT/LONG SOURCE
    lahso_coords_source: Mapped[str | None] = mapped_column(String(16))
    ...
    lahso_coords_date: Mapped[datetime.date | None] = mapped_column(Date)
...
class AirportRemark(Base):
    """Free-form remarks attached to an Airport record."""
    ...
    def __repr__(self) -> str:
        """Short debug representation."""
        remark_preview = self.remark[:13] if self.remark else ""
        return f"<Airport Remark(airport='{self.airport}', element_name='{self.remark_element_name}', remark='{remark_preview}...')>"


class RunwayRemark(Base):
    """Free-form remarks attached to a Runway record."""
    ...
    def __repr__(self) -> str:
        """Short debug representation."""
        remark_preview = self.remark[:13] if self.remark else ""
        return f"<Runway Remark(runway='{self.runway}', element_name='{self.remark_element_name}', remark='{remark_preview}...')>"


class RunwayEndRemark(Base):
    """Free-form remarks attached to a RunwayEnd record."""
    ...
    def __repr__(self) -> str:
        """Short debug representation."""
        remark_preview = self.remark[:13] if self.remark else ""
        return f"<RunwayEnd Remark(runway_end='{self.runway_end}', element_name='{self.remark_element_name}', remark='{remark_preview}...')>"


class AttendanceSchedule(Base):
    """Attendance schedules for an Airport."""
    ...
    def __repr__(self) -> str:
        """Short debug representation."""
        schedule_preview = (
            self.attendance_schedule[:13] if self.attendance_schedule else ""
        )
        return f"<Attendance Schedule(airport='{self.airport}', sequence_number='{self.sequence_number}', attendance_schedule='{schedule_preview}')>"
